...
class Logger:
    ...
    def log_update(self, new_record, old_rid, old_indirection, old_schema, lock):
        lock.acquire()
        self.current_transaction+=1
        self.logger_transaction = {'id': self.current_transaction, 'operation': "update", 'key': new_record.key, 'old_rid': old_rid, 
    ...
    def log_delete(self, record, lock):
        lock.acquire()
        self.current_transaction+=1
        self.logger_transaction = {'id': self.current_transaction, 'operation': "delete", 'rid': record.rid, 'key': record.key , 'commited': False, 'aborted': False, 'table': self.table_name}
