...
class Logger:
    ...
    def log_update(self, record, new_record):
        ...
        self.logger_transaction = {'id': self.current_transaction, 'operation': "update", 'key': record.key, 'old_rid': record.rid, 'indirection': record.columns[indirection_column], 'schema': record.columns[-1], 'rid': new_record.rid , 'query': new_record.columns[:updated_columns], 'commited': False, 'aborted': False}
        with open(self.path, 'wb') as file:
            pickle.dump(self.logger_transaction, file)
